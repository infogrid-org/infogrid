 ...
 */
public abstract class AbstractWebMeshObjectsToView
        extends
            AbstractMeshObjectsToView
        implements
            WebMeshObjectsToView
{
    private static final Log log = Log.getLogInstance(AbstractWebMeshObjectsToView.class ); // our own, private logger
     ...
     */
    protected AbstractWebMeshObjectsToView(
            MeshObject                subject,
            ...
            TraversalPath childArrivedAt )
    {
        ...
        if( theViewletTypeName != null ) {
            HTTP.appendArgumentToUrl(buf, WebMeshObjectsToView.LID_FORMAT_ARGUMENT_NAME, WebMeshObjectsToView.VIEWLET_PREFIX + theViewletTypeName );
        }
        ...
        try {
            ...
            if( traversalArgs != null && traversalArgs.length > 0 ) {
                for( int i=0 ; i<traversalArgs.length ; ++i ) {
                    HTTP.appendArgumentToUrl(buf, WebMeshObjectsToView.LID_TRAVERSAL_ARGUMENT_NAME, traversalArgs[i] );
                }
            }
            ...
            Deque<WebViewedMeshObjects> viewedMeshObjectsStack )
    {
        ...
        if( viewedMeshObjectsStack != null ) {
            ...
            for( WebViewedMeshObjects current : viewedMeshObjectsStack ) {
