...
{
    private static final Log log = Log.getLogInstance( DefaultWebMeshObjectsToViewFactory.class ); // our own, private logger
            ...
            Context                                         c )
    {
        ...
        return new DefaultWebMeshObjectsToViewFactory(
     ...
     */
    @Override
    public DefaultWebMeshObjectsToView obtainFor(
            MeshObject key )
    {
            ...
            FactoryException
    {
        ...
        try {
            ...
            // Viewlet type
            viewletTypeName = determineArgumentForMulti( request, WebMeshObjectsToView.LID_FORMAT_ARGUMENT_NAME, WebMeshObjectsToView.VIEWLET_PREFIX );

            // MIME type
            mimeType = determineArgumentForMulti( request, WebMeshObjectsToView.LID_FORMAT_ARGUMENT_NAME, WebMeshObjectsToView.MIME_PREFIX );

            // traversal
            String [] traversalTerms = request.getMultivaluedUrlArgument( WebMeshObjectsToView.LID_TRAVERSAL_ARGUMENT_NAME );
            path = null; // default
            spec = null;
            ...
            // ViewletState
            viewletState = DefaultWebViewletStateEnum.fromRequest( request );

            // ViewletStateTransition
            viewletTransition = DefaultWebViewletStateTransitionEnum.fromRequest( request );

            // remaining arguments (but not lid-include) are viewletPars
            viewletPars = null;
            for( String key : request.getUrlArguments().keySet() ) {
                if( WebMeshObjectsToView.LID_FORMAT_ARGUMENT_NAME.equals( key )) {
                    continue;
                }
    ...
    /**
     * Create a sequence of MeshObjectsToView that correspond to a hierarchical request
     * enclosed in this SaneRequest.
     * 
     * @param request the SaneUrl
     * @return the created MeshObjectsToView
     * @throws FactoryException thrown if the MeshObjectsToView could not be created
     */
    @Override
    public Deque<? extends DefaultWebMeshObjectsToView> obtainStackFor(
            SaneUrl request )
        throws
            FactoryException
    {
        try {
            ArrayDeque<DefaultWebMeshObjectsToView> ret = new ArrayDeque<>();
            String originalAbsoluteContext = request.getAbsoluteContextUri();

            SaneUrl current = request;
            while( current != null ) {
                DefaultWebMeshObjectsToView found = obtainFor( current );
    ...
    /**
     * The web application's (relative) context path.
     */
